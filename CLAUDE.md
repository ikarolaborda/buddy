...
- `php artisan horizon` — The worker: one fixed-size Horizon supervisor per queue lane (ADR 0014). Azure runs it through `docker/production/horizon-entrypoint.sh`, which bounds the shutdown when the platform severs Redis
